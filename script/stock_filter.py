...
def get_stock_history(stock_code, period="5y", use_cache=True, cache_dir=None):
    ...
    Returns:
        pandas.DataFrame: 股票历史数据，包含OHLCV等信息
    """
    try:
        import os
        import pandas as pd
        ...
        history_data = xueqiu_stock.kline(symbol=stock_code, days=period, period='day')

        # 检查是否成功获取到数据
        if history_data is None or 'item' not in history_data or not history_data['item']:
            print(f"没有获取到股票 {stock_code} 的历史数据")
            return None

        # 从JSON数据创建DataFrame
        columns = history_data['column']
        items = history_data['item']
        
        # 检查是否包含必要的列
        required_fields = ['timestamp', 'open', 'high', 'low', 'close', 'volume']
        for field in required_fields:
            if field not in columns:
                print(f"股票 {stock_code} 数据中缺少必要字段: {field}")
                return None
        ...
        if len(df) < 20:  # 至少需要20个交易日的数据
            print(f"股票 {stock_code} 的历史数据不足 (仅有 {len(df)} 个交易日)")
            return None
            
        # 数据清洗：处理缺失值和异常值
        # 填充价格缺失值
        for col in ['Open', 'High', 'Low', 'Close']:
            df[col] = df[col].replace(0, np.nan)  # 将0值替换为NaN
            df[col] = df[col].ffill()  # 用前一个有效值填充
        
        # 处理成交量为0或NaN的情况
        df['Volume'] = df['Volume'].replace(0, np.nan)
        df['Volume'] = df['Volume'].ffill()  # 使用ffill()代替fillna(method='ffill')
        
        # 如果仍有缺失值，填充为最小非零值
        min_volume = df['Volume'].replace(0, np.nan).min()
        if not np.isnan(min_volume):
            df['Volume'] = df['Volume'].fillna(min_volume)
                ...
                if pullback_idx >= (len(prices) - 84):
                    ...
                    if pullback_idx + 10 < len(prices):
                        ...
                        if min(after_pullback) >= pullback_price * 0.97:
                            # 回测后有企稳迹象
                            last_5_days = prices[pullback_idx + 5:pullback_idx + 10] if pullback_idx + 10 < len(
                                prices) else []
...
def filter_stocks(min_pe=5, max_pe=50, market_type="US", strategies=None, output_dir=None, parallel=False):
    ...
    if output_dir:
        ...
        # 为每个策略创建一个CSV文件
        file_handlers["volume_surge"] = open(os.path.join(output_dir, f"strategy1_volume_surge_{timestamp}.csv"), "w",
                                             newline='', encoding="utf-8")
        file_handlers["pullback_50"] = open(os.path.join(output_dir, f"strategy2_pullback_50_{timestamp}.csv"), "w",
                                            newline='', encoding="utf-8")
        file_handlers["initial_pullback_20"] = open(
            os.path.join(output_dir, f"strategy3_initial_pullback_20_{timestamp}.csv"), "w", newline='',
            encoding="utf-8")
        file_handlers["all_strategies"] = open(os.path.join(output_dir, f"all_strategies_{timestamp}.csv"), "w",
                                               newline='', encoding="utf-8")
